...
import { firebaseConfig, announcementFirebaseConfig, activityFirebaseConfig } from "./firebase-config.js";
...
async function syncAnnouncementPermissions(){
  ...
  try{
    ...
    console.error(err); setAnnouncementSyncStatus("同步失敗："+friendly(err),"error"); toast("同步失敗："+friendly(err));
...
function myAllowedSystemIds(){
  ...
  const perms=profile?.permissions||{}; return systems.filter(s=>perms[s.id]===true).map(s=>s.id);
}
